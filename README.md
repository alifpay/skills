...
|---|---|
| [`review-error-classification`](./review-error-classification/SKILL.md) | Error branches in transactional flows. Infra errors (Redis/RabbitMQ/Kafka/DB/HTTP/timeout) must not become final `FAILED` status; unknown outcomes must stay `PROCESSING`; Unknown sub-kinds (`null_status`, `empty_body`, `unparseable`, `unknown_enum`, `unknown_id`, `unexpected_code`, `signature_fail`) each get their own alert; responses to callers must match committed state; **a failure to write to your own DB trips an inbound circuit breaker (halts new transactional traffic) and pages on-call instead of marking the row FAILED**. |
| [`review-external-api`](./review-external-api/SKILL.md) | Outbound calls to third-party / provider APIs. Idempotency key stability across retries (no random-suffix tricks), response-code classification, schema validation + contract-drift detection, refund/reversal guards, two-step auth+confirm amount validation, reconciliation, versioning, and **deployment of version bumps to a live integration (blue-green / staged rollout, shadow / parallel-run with no side effects, one-toggle rollback rehearsed in staging, stable idempotency-key format across the cutover, drain wait by oldest in-flight transaction)**. |
| [`review-sync-transactional`](./review-sync-transactional/SKILL.md) | Synchronous money-moving HTTP/gRPC endpoints. Context ownership, timeouts, client-disconnect handling, rate limiting, rollback endpoints, admin-route separation, **API versioning discipline (no breaking changes in the same version)**, and **server-side idempotency on transactional intake (`Idempotency-Key` required, principal-scoped uniqueness, body-hash match, cached-response replay, concurrent-key serialization, documented TTL)**. |
